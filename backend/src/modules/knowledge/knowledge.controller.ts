...
export class KnowledgeController {
  ...
  async list(
    ...
    const { items, pagination } = await this.knowledgeService.list(dto);
    return {
  ...
  async listCitations(
    ...
    const items = await this.knowledgeService.listCitationsFor(params.id);
    return { items };
  }
  ...
  async create(
    ...
    const entry = await this.knowledgeService.create(user.sub, dto);
    return {
      success: true,
  ...
  async findOne(
    ...
    const entry = await this.knowledgeService.findOneOrThrow(params.id);
  ...
  async update(
    ...
    const existing = await this.knowledgeService.findOneOrThrow(params.id);
    ...
    const updated = await this.knowledgeService.update(params.id, dto);
    return {
      success: true,
  ...
  async remove(
    ...
    await this.knowledgeService.remove(params.id);
  }
}