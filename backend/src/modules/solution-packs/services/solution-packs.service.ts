...
export class SolutionPacksService {
   ...
   */
  async listCatalog(options: BrowsePacksOptions = {}): Promise<SolutionPack[]> {
   ...
   */
  async previewInstall(packSlug: string): Promise<PackInstallPreview> {
    const pack = await this.getBySlug(packSlug);
    const tenantId = this.tenantContext.tenantId;
    const tenantTier = await this.resolveTenantTier();
   ...
   */
  async listInstalled(): Promise<TenantInstalledPack[]> {
    const tenantId = this.tenantContext.tenantId;
   ...
   */
  async getInstallHistory(limit = 50): Promise<PackInstallationLogEntry[]> {
    const tenantId = this.tenantContext.tenantId;
  ...
  private async resolveTenantTier(): Promise<PackTierRequired> {
    const tenantId = this.tenantContext.tenantId;
