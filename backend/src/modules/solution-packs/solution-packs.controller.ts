...
export class SolutionPacksController {
  ...
  async list(@Query() query: ListSolutionPacksDto) {
    return this.packs.listCatalog({
  ...
  async listInstalled() {
    return this.packs.listInstalled();
  }
  ...
  async installHistory(@Query('limit') limit?: string) {
    const parsed = limit ? Math.min(200, Math.max(1, parseInt(limit, 10))) : 50;
    return this.packs.getInstallHistory(Number.isFinite(parsed) ? parsed : 50);
  }
  ...
  async preview(@Param('slug') slug: string) {
    return this.packs.previewInstall(slug);
  }
  ...
  async install(
    ...
    const performedById = req.user?.sub ?? req.user?.userId ?? null;
  ...
  async uninstall(@Param('slug') slug: string, @Req() req: AuthedRequest) {
    const performedById = req.user?.sub ?? req.user?.userId ?? null;
