...
export class ActionAuthorizationGuard implements CanActivate {
  ...
  constructor(
    ...
    // Injected for DI completeness (some throttler setups use it).
    private readonly _throttler: ThrottlerStorage,
  ) {}
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    ...
    const tenantId = user.tenantId ?? this.tenantContext.tenantId;
    if (!tenantId) {
      throw new ForbiddenException({
