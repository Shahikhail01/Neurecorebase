...
@Module({
  ...
  providers: [
    ...
    // ThrottlerStorage is global — provide a NoOp fallback so the guard
    // can depend on the interface without re-binding the global ThrottlerModule.
    {
      provide: ThrottlerStorage,
      useValue: {
        increment: () => Promise.resolve({ totalHits: 0, timeToExpire: 0 }),
      },
    },
  ],
  exports: [
