...
export class AIActionsController {
  ...
  async listAvailable(
    ...
    const items = await this.aiActions.listAvailable(user.sub, entityType);
    return {
      items,
      pagination: {
        page: 1,
  ...
  async execute(
    ...
    return this.aiActions.execute(user.sub, dto);
  }
  ...
  async getInvocation(
    ...
    return this.aiActions.getInvocation(invocationId, user.sub);
  }
