...
export class MarketplaceService {
  ...
  async browseTab(args: {
    ...
    switch (args.tab) {
      ...
      case 'installed':
        return this.browseInstalled(args);
      default:
        return [];
    }
  }
  ...
  private async browsePacks(args: {
    q?: string;
    installedOnly?: boolean;
  }): Promise<MarketplaceItem[]> {
    const packs = await this.solutionPacks.listCatalog({
      q: args.q,
  ...
  private async browseInstalled(_args: {
    q?: string;
  }): Promise<MarketplaceItem[]> {
    const installed = await this.solutionPacks.listInstalled();
    return installed.map((i) => ({
      id: i.solutionPackId,
      tab: 'installed',
      slug: i.packSlug,
      name: i.packSlug,
      shortDescription: `Installed ${i.installedAt.slice(0, 10)} (v${i.packVersion})`,
      icon: 'check-circle',
      color: '#22c55e',
      installed: true,
      meta: {
