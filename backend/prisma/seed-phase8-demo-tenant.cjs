...
async function upsertTenant() {
  ...
  if (!tenant) {
    ...
    await p.tenant.update({
      where: { id: tenant.id },
      data: { plan: TENANT_PLAN, status: 'ACTIVE' },
    });
    console.log(`  · tenant ${tenant.slug} already exists — refreshed to ${TENANT_PLAN}`);
  }
  return tenant;
}
...
async function createRetailStores(tenantId, ownerId) {
  ...
  // Universal EntityState + EntityOwnership for every retail-store.
  for (const s of out) {
    await p.entityState.upsert({
      ...
      // EntityState uses @@unique([entityId]) so this may already exist.
    });
    ...
    const existingTag = await p.entityLabel.findFirst({
      where: { tenantId, entityId: s.departmentId, labelName: s.type },
    });
    if (!existingTag) {
  ...
  console.log(`  ✓ ${out.length} EntityState + EntityOwnership + EntityHealth rows written`);
...
async function createRetailAgents(tenantId, stores, ownerId) {
  let count = 0;
  // For each store, create 2-3 agents.
      ...
      if (!existing) {
        await p.agent.create({
          data: {
            tenantId,
            name: a.name,
            type: idx === 0 ? 'EXECUTIVE' : 'FUNCTIONAL',
            status: 'ACTIVE',
  ...
  // Favorites — pin the first 5 stores for the owner.
  for (let i = 0; i < Math.min(5, stores.length); i++) {
    const existing = await p.userFavorite.findFirst({
      where: { userId: ownerId, entityId: stores[i].departmentId },
    });
    if (!existing) {
...
async function createWorkflows(tenantId, ownerId, stores) {
  // Create one workflow per template, attached to the first store.
...
async function createDefaultLayouts(tenantId, ownerId) {
  // One default Insights layout per retail-store entity.
  ...
  for (const s of stores) {
    const existing = await p.workspaceLayout.findFirst({
      where: { userId: ownerId, entityId: s.id },
    });
    if (existing) continue;
    await p.workspaceLayout.create({
      data: {
        ...
        widgets: [
          ...
          { widgetId: 'retail-kpi:conversion-rate', x: 0, y: 2, w: 6, h: 4 },
          { widgetId: 'retail-kpi:inventory-heatmap', x: 6, y: 3, w: 6, h: 4 },
          { widgetId: 'retail-kpi:sales-by-hour', x: 0, y: 6, w: 12, h: 4 },
        ],
        isDefault: true,
      },
    });
