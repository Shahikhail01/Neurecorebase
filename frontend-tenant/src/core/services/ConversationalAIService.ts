  ...
  async sendMessage(
    ...
    try {
      const response = await restClient.post<{
        ...
        chartType?:     ChatMessage['metadata']['chartType'];
        chartData?:     ChatMessage['metadata']['chartData'];
        suggestions?:   string[];
      }>(API_ENDPOINT, {
        message,
      ...
      const rawMeta = response.data as Record<string, unknown> | null | undefined;
      const { cleanedReply, ...metadata } = this._parseMetadata(reply, rawMeta ?? null);
